...
export function BacktestPage() {
  ...
  return <div className="p-5">
    ...
    <Card className="mb-3 grid grid-cols-2 gap-3 p-4 xl:grid-cols-6"><Select label="Plataforma" value={platformId} onChange={setPlatformId} options={platforms}/><Select label="Game Strategy" value={gameStrategyId} onChange={setGameStrategyId} options={gameStrategies}/><Select label="Bet Strategy" value={betStrategyId} onChange={setBetStrategyId} options={betStrategies}/><Select label="Plano de aposta" value={betPlanId} onChange={setBetPlanId} options={betPlans}/><NumberField label="Banca inicial (R$)" value={bankroll} onChange={setBankroll}/><NumberField label="Rodadas" value={limit} onChange={setLimit}/></Card>
...
function NumberField({label,value,onChange}:{label:string;value:number;onChange(value:number):void}) { return <label><span className="label">{label}</span><input className="mt-2 w-full rounded-md border-line bg-canvas text-xs" type="number" min="10" value={value} onChange={e=>onChange(Number(e.target.value))}/></label>; }
