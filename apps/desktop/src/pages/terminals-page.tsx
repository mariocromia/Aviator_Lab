...
function OperationCombinationsEditor({combinations,onChange,betStrategies,betPlans}:{combinations:TerminalOperationCombination[];onChange(value:TerminalOperationCombination[]):void;betStrategies:Array<{id:string;name:string}>;betPlans:Array<{id:string;name:string}>}){
  ...
  const add=()=>onChange([...combinations,{id:crypto.randomUUID(),name:`Combinação ${combinations.length+1}`,priority:(combinations.length+1)*10,enabled:true,triggerType:'PATTERN',pattern:'W',betStrategyId:betStrategies[0]?.id??'',lossReentryType:'PATTERN',lossReentryPattern:'W',lossReentryBetStrategyId:null,betPlanId:betPlans[0]?.id??'',behavior:'REPEAT_UNTIL_LOSS'}]);
  return <fieldset className="col-span-2 rounded-lg border border-line bg-canvas p-4"><legend className="px-1 text-[10px] font-bold uppercase tracking-wider text-muted">Combinações operacionais</legend>
    <div className="flex items-start justify-between gap-4"><p className="max-w-2xl text-[10px] leading-5 text-muted">Digite diretamente sequências como LLLWWL, LW ou W, ou selecione uma estratégia de entrada. A reentrada após LOSS pode ter uma condição própria.</p><Button type="button" onClick={add} className="shrink-0 bg-brand text-white"><Plus size={12}/>Adicionar combinação</Button></div>
    <div className="mt-3 space-y-3">{combinations.map((combination,index)=><div key={combination.id} className="rounded-md border border-line bg-panel p-3">
      ...
      <div className="mt-3 grid grid-cols-2 gap-3"><label><span className="label">Gatilho inicial</span><select className="mt-1.5 w-full" value={combination.triggerType??'BET_STRATEGY'} onChange={event=>update(index,{triggerType:event.target.value as TerminalOperationCombination['triggerType']})}><option value="PATTERN">Digitar sequência W/L</option><option value="BET_STRATEGY">Usar estratégia de entrada</option></select></label>{(combination.triggerType??'BET_STRATEGY')==='PATTERN'?<label><span className="label">Sequência para entrar</span><input className="mt-1.5 w-full font-mono uppercase" value={combination.pattern??''} onChange={event=>update(index,{pattern:normalizePattern(event.target.value)})} placeholder="Ex.: LLLWWL, LW ou W"/></label>:<label><span className="label">Estratégia de entrada</span><select className="mt-1.5 w-full" value={combination.betStrategyId} onChange={event=>update(index,{betStrategyId:event.target.value})}>{betStrategies.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label>}</div>
