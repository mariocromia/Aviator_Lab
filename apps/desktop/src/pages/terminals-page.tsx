...
import { BarChart3, Bot, Clock3, Copy, FolderOpen, Gamepad2, MousePointer2, Pause, Play, Plus, ReceiptText, RefreshCw, RotateCcw, Save, Search, Settings2, SquareTerminal, Trash2, Zap } from 'lucide-react';
import { Area, AreaChart, Bar, BarChart, CartesianGrid, Line, LineChart, ResponsiveContainer, Tooltip, XAxis, YAxis } from 'recharts';
import type { NormalizedRound, ScreenProfile, Terminal, TerminalHistoryItem, TerminalLossStreakStats, TerminalOperationCombination, TerminalPreset, TerminalRuntime } from '@aviator/shared';
...
export function TerminalsPage() {
  ...
  const [globalMousePaused,setGlobalMousePaused]=useState(true);
  const terminals = store.terminals.filter(terminal => terminal.name.toLowerCase().includes(query.toLowerCase()));
  useEffect(()=>{let active=true;const load=()=>void window.aviator.getScreenAutomationStatus().then(result=>{if(active&&result.ok&&result.data)setGlobalMousePaused(result.data.paused)});load();const timer=window.setInterval(load,2_000);return()=>{active=false;window.clearInterval(timer)}},[]);
  async function toggleMouse(terminalId:string,enabled:boolean){store.clearError();const result=await window.aviator.setTerminalScreenAutomationEnabled(terminalId,enabled);if(!result.ok){window.alert(result.error??'Não foi possível alterar o controle do mouse.');return}setGlobalMousePaused(false);await store.refresh();}
  ...
  return <div className="p-5">
    ...
    <div className="grid gap-3 xl:grid-cols-2">
      {terminals.map(terminal => {
        ...
        const mouseEnabled=terminal.mode==='ASSISTED'&&Boolean(screenProfile)&&!globalMousePaused&&runtime?.screenControllerState.paused===false;
        ...
        const lossStreakStats=store.terminalLossStreakStats[terminal.id];
        return <Card key={terminal.id} className="overflow-hidden">
          <div className="flex items-center border-b border-line px-4 py-3">
            ...
            <div className="min-w-0"><h3 className="truncate text-sm font-semibold">{terminal.name}</h3><div className="mt-1 flex items-center gap-2"><Badge tone={isPaused ? 'warning' : 'success'}>{translateTerminalStatus(runtime?.status??(terminal.paused?'PAUSED':'RUNNING'))}</Badge><Badge tone={mouseEnabled?'success':'neutral'}>{mouseEnabled?'MOUSE ATIVO':'MOUSE OFF'}</Badge></div></div>
            <div className="ml-auto flex shrink-0 items-center gap-1 pl-3">
              <TerminalIconButton title={isPaused?'Retomar Terminal':'Pausar Terminal'} onClick={()=>void store.setTerminalPaused(terminal.id,!isPaused)} disabled={isUpdating} tone={isPaused?'success':'warning'}>{isPaused?<Play size={13}/>:<Pause size={13}/>}</TerminalIconButton>
              ...
              <TerminalIconButton title="Abrir simulador desta plataforma" onClick={()=>void window.aviator.openBetSimulator(terminal.id)} tone="success"><Gamepad2 size={13}/></TerminalIconButton>
              <TerminalIconButton title={!screenProfile?'Configure o bot visual antes de ativar o mouse':terminal.mode!=='ASSISTED'?'Altere para ASSISTED antes de ativar o mouse':mouseEnabled?'Desativar controle do mouse neste Terminal':'Ativar controle do mouse neste Terminal'} onClick={()=>void toggleMouse(terminal.id,!mouseEnabled)} disabled={!screenProfile||terminal.mode!=='ASSISTED'} tone={mouseEnabled?'active':'neutral'}><MousePointer2 size={13}/></TerminalIconButton>
          ...
          <div className="space-y-2 p-4 text-[11px]">
            ...
            <Row label="Fonte W/L" value={strategySource?`Terminal ${strategySource.name}`:'Jogo próprio'} extra={strategySource?(terminal.strategySourceMode==='ALL_SOURCE_SIGNALS'?'apostar todos os W/L, inclusive sem aposta':terminal.strategySourceMode==='BET_EXECUTIONS'?'somente apostas executadas pela fonte':terminal.strategySourceMode==='BET_ENTRIES'?'somente entradas iniciais aprovadas':'sinais do terminal'):(strategy?.name??'—')}/>
            ...
            <Row label="Início financeiro" value={<span className="flex items-center gap-2"><span>{new Date(terminal.bankrollStartAt??terminal.createdAt).toLocaleString('pt-BR')}</span>{lossStreakStats&&<LossInsightsButton stats={lossStreakStats}/>}</span>}/>
            <Row label="Tempo de banca" value={<BankrollElapsedTime startedAt={terminal.bankrollStartAt??terminal.createdAt}/>}/>
            ...
            {(runtime?.betStrategyRuntime.postWinSkipRemaining??0)>0&&<Row label="Espera após WIN" value={`${runtime?.betStrategyRuntime.postWinSkipRemaining} SINAIS RESTANTES`}/>} 
            ...
            <Row label="Controle do mouse" value={mouseEnabled?'ATIVO':'DESATIVADO'} badge="decision"/>
...
function ScreenProfileModal({ terminal, profile, onClose }: { terminal: Terminal | null; profile: ScreenProfile | null; onClose(): void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const slot = (name: 'bet1' | 'bet2') => { const current=profile?.[name]??fallback; return { enabled: form.get(`${name}.enabled`) === 'on', amountCents: current.amountCents, cashout: current.cashout, amount: { x: number(`${name}.amount.x`), y: number(`${name}.amount.y`) }, cashoutField: { x: number(`${name}.cashout.x`), y: number(`${name}.cashout.y`) }, action: { x: number(`${name}.action.x`), y: number(`${name}.action.y`) } }; };
    const saved = await store.saveScreenProfile({ terminalId: terminal.id, name: String(form.get('name')), resolutionWidth: number('resolutionWidth'), resolutionHeight: number('resolutionHeight'), windowTitle: String(form.get('windowTitle')).trim() || null, inactivityBet: profile?.inactivityBet, bet1: slot('bet1'), bet2: slot('bet2') });
...
function ScreenSlotFields({ slot, title, value }: { slot: 'bet1' | 'bet2'; title: string; value: ScreenProfile['bet1'] }) {
  return <fieldset className="mt-4 rounded-md border border-line p-4"><div className="mb-3 flex items-center justify-between"><legend className="text-xs font-semibold">{title}</legend><label className="flex items-center gap-2 text-[10px] text-muted"><input name={`${slot}.enabled`} type="checkbox" defaultChecked={value.enabled}/> Ativa</label></div><div className="grid grid-cols-2 gap-3"><div className="col-span-2 rounded-md border border-brand/20 bg-brand/5 p-2 text-[9px] text-blue-200">VALOR E SAÍDA: DINÂMICOS DO PLANO BASE/GALE</div><CoordinateFields prefix={`${slot}.amount`} label="Campo valor" value={value.amount}/><CoordinateFields prefix={`${slot}.cashout`} label="Campo cashout" value={value.cashoutField}/><CoordinateFields prefix={`${slot}.action`} label="Botão ação" value={value.action}/></div></fieldset>;
}
...
function BankrollElapsedTime({startedAt}:{startedAt:string}){
  const[now,setNow]=useState(()=>Date.now());
  useEffect(()=>{setNow(Date.now());const timer=window.setInterval(()=>setNow(Date.now()),1_000);return()=>window.clearInterval(timer)},[startedAt]);
  return <span className="font-mono">{formatElapsedTime(Math.max(0,now-Date.parse(startedAt)))}</span>;
}

function formatElapsedTime(milliseconds:number){const totalSeconds=Math.floor(milliseconds/1_000);const days=Math.floor(totalSeconds/86_400);const hours=Math.floor(totalSeconds%86_400/3_600);const minutes=Math.floor(totalSeconds%3_600/60);const seconds=totalSeconds%60;const clock=[hours,minutes,seconds].map(value=>String(value).padStart(2,'0')).join(':');return days>0?`${days}d ${clock}`:clock;}
...
function formatLossDuration(value:number|null){
  if(value===null||!Number.isFinite(value))return'SEM DADOS';
  const totalMinutes=Math.max(0,Math.floor(value/60_000));const days=Math.floor(totalMinutes/1_440);const hours=Math.floor(totalMinutes%1_440/60);const minutes=totalMinutes%60;
  return days>0?`${days}d ${hours}h`:hours>0?`${hours}h ${minutes}min`:`${minutes}min`;
}

function LossInsightsButton({stats}:{stats:TerminalLossStreakStats}){
  const[popover,setPopover]=useState<{left:number;top:number}|null>(null);
  function open(element:HTMLButtonElement){const rect=element.getBoundingClientRect();const width=440;const estimatedHeight=430;const left=Math.max(8,Math.min(rect.right-width,window.innerWidth-width-8));const below=rect.bottom+6;const top=below+estimatedHeight<=window.innerHeight?below:Math.max(8,rect.top-estimatedHeight-6);setPopover({left,top});}
  const lastAt=stats.lastExceededGalesAt?Date.parse(stats.lastExceededGalesAt):null;
  const items=[
    {label:'Loss consecutivos máximos na banca',value:`L${stats.bankrollMax}`},
    {label:'Loss consecutivos máximos no histórico',value:`L${stats.historyMax}`},
    {label:`Sequências acima de ${stats.galeLimit} ${stats.galeLimit===1?'Gale':'Gales'} na banca`,value:String(stats.bankrollExceededGales)},
    {label:`Sequências acima de ${stats.galeLimit} ${stats.galeLimit===1?'Gale':'Gales'} no histórico`,value:String(stats.historyExceededGales)},
    {label:'Tempo desde a última acima dos Gales',value:lastAt===null?'NUNCA OCORREU':formatLossDuration(Date.now()-lastAt),detail:lastAt===null?undefined:new Date(lastAt).toLocaleString('pt-BR')},
    {label:'Tempo médio entre ocorrências',value:formatLossDuration(stats.averageExceededGalesIntervalMs),detail:'todo o histórico'},
    {label:`Vezes em que o teto histórico L${stats.historyMax} ocorreu`,value:String(stats.historyMaxOccurrences)},
    {label:`Vezes em que o teto da banca L${stats.bankrollMax} ocorreu`,value:String(stats.bankrollMaxOccurrences)},
    {label:'Menor valor atingido pela banca',value:money(stats.bankrollMinCents)},
    {label:'Maior valor atingido pela banca',value:money(stats.bankrollMaxCents)}
  ];
  return <><button type="button" className="rounded border border-danger/50 bg-danger/15 px-2 py-0.5 font-mono text-[9px] font-black tracking-wider text-danger transition hover:bg-danger/25 focus:outline-none focus:ring-1 focus:ring-danger" aria-label="Abrir análise completa de Loss" onMouseEnter={event=>open(event.currentTarget)} onMouseLeave={()=>setPopover(null)} onFocus={event=>open(event.currentTarget)} onBlur={()=>setPopover(null)}>LOSS</button>{popover&&createPortal(<div role="tooltip" className="pointer-events-none fixed z-[9999] w-[440px] rounded-lg border border-danger/35 bg-elevated p-4 shadow-2xl" style={popover}><div className="mb-3 flex items-center justify-between border-b border-line pb-2"><div><div className="text-xs font-bold text-danger">ANÁLISE COMPLETA DE LOSS</div><div className="mt-0.5 text-[9px] text-muted">Limite da estratégia: {stats.galeLimit} {stats.galeLimit===1?'Gale':'Gales'}</div></div><span className="rounded bg-danger/15 px-2 py-1 font-mono text-[9px] font-bold text-danger">HISTÓRICO + BANCA</span></div><div className="grid grid-cols-2 gap-2">{items.map(item=><div key={item.label} className="rounded-md border border-line bg-canvas px-3 py-2"><div className="text-[9px] leading-4 text-muted">{item.label}</div><div className="mt-1 font-mono text-[12px] font-bold text-ink">{item.value}</div>{item.detail&&<div className="mt-0.5 font-mono text-[8px] text-muted">{item.detail}</div>}</div>)}</div><div className="mt-3 text-[8px] leading-4 text-muted">Cada sequência é contada uma única vez quando alcança mais Loss que o número de Gales configurado no plano para LOSS.</div></div>,document.body)}</>;
}

function Row({ label, value, extra, badge }: { label: string; value: ReactNode; extra?: string; badge?: 'strategy' | 'decision' }) {
  const textValue=typeof value==='string'?value:'';
  const tone = badge === 'decision' ? textValue === 'ENTRAR' ? 'success' : textValue === 'PAUSAR' ? 'danger' : 'neutral' : badge === 'strategy' ? ['AGUARDANDO RESULTADO','CONTINUANDO ATÉ LOSS'].includes(textValue)||textValue.startsWith('APOSTA ') ? 'success' : textValue === 'AGUARDANDO LIBERAÇÃO'||textValue.startsWith('ENTRADA BLOQUEADA') ? 'warning' : 'neutral' : null;
...
function TerminalModal({ open, terminal, onClose }: { open: boolean; terminal: Terminal | null; onClose(): void }) {
  ...
  return <Modal title={terminal ? `Configurar ${terminal.name}` : 'Novo Terminal'} open={open} onClose={onClose}>
    <form onSubmit={submit} className="grid grid-cols-2 gap-4 p-5">
      ...
      <Field label="Dados recebidos da referência"><select name="strategySourceMode" defaultValue={terminal?.strategySourceMode??'GAME_SIGNALS'}><option value="GAME_SIGNALS">Sinais W/L internos</option><option value="ALL_SOURCE_SIGNALS">Apostar todos os W/L da fonte — inclusive sem aposta</option><option value="BET_EXECUTIONS">Somente apostas executadas pela fonte</option><option value="BET_ENTRIES">Somente entradas iniciais aprovadas</option></select><small>“Todos os W/L” arma a próxima rodada para cada sinal do filtro, incluindo gatilhos sem aposta, WINs consecutivos e WIN isolado entre LOSS.</small></Field>
      ...
      <Field label="Sinais sem apostar após WIN" className="col-span-2"><input name="postWinSkipSignals" type="number" min="0" max="10000" step="1" defaultValue={terminal?.postWinSkipSignals??0}/><p className="mt-1 text-[9px] text-muted">Continua gerando e registrando sinais W/L, mas bloqueia novas apostas até consumir esta quantidade. Zero desativa.</p></Field>
...
function translateOperationalState(runtime:TerminalRuntime|undefined,terminal?:Terminal){if(runtime?.galeRuntime.active){const stage=runtime.galeRuntime.currentStage;if(runtime.galeRuntime.awaitingDynamicFirstGale){const remaining=Math.max(0,(runtime.galeRuntime.triggerLossStreakTarget??1)-runtime.resultAnalyzerState.currentLossStreak);return`G${stage} AGUARDANDO MAIS ${remaining} LOSS`;}if(stage>0&&runtime.galeRuntime.entryConfirmed===false&&(runtime.galeRuntime.preparedLegAmountsCents?.length??0)===0)return`G${stage} AGUARDANDO CONFIRMAÇÃO`;if(terminal&&['BET_EXECUTIONS','BET_ENTRIES','ALL_SOURCE_SIGNALS'].includes(terminal.strategySourceMode))return stage===0?'BASE ARMADA • PRÓXIMA RODADA':`G${stage} ARMADO • PRÓXIMA RODADA`;if(runtime.galeRuntime.followUp&&runtime.galeRuntime.followUpBehavior==='REPEAT_UNTIL_LOSS')return'CONTINUANDO ATÉ LOSS';return`APOSTA ${stage===0?'BASE':`G${stage}`} AGUARDANDO`;}const blocked=terminal&&runtime?terminal.entryBlockPatterns.find(pattern=>runtime.resultAnalyzerState.recentPattern.toUpperCase().endsWith(pattern)):null;if(blocked)return`ENTRADA BLOQUEADA • ${blocked}`;return terminal?.strategySourceMode==='ALL_SOURCE_SIGNALS'?'AGUARDANDO PRÓXIMO W/L DA FONTE':terminal?.strategySourceMode==='BET_ENTRIES'?'AGUARDANDO ENTRADA APROVADA DA FONTE':terminal?.strategySourceMode==='BET_EXECUTIONS'?'AGUARDANDO OPERAÇÃO DA FONTE':translateGameState(runtime?.gameStrategyRuntime.state??'SEARCH_TRIGGER');}
function translateBetCycle(runtime:TerminalRuntime|undefined,terminal?:Terminal){if(!runtime?.galeRuntime.active)return'SEM CICLO ATIVO';if(runtime.galeRuntime.awaitingDynamicFirstGale){const remaining=Math.max(0,(runtime.galeRuntime.triggerLossStreakTarget??1)-runtime.resultAnalyzerState.currentLossStreak);return`GALE 1 LIBERA APÓS MAIS ${remaining} LOSS`;}if(runtime.galeRuntime.currentStage>0&&runtime.galeRuntime.entryConfirmed===false&&(runtime.galeRuntime.preparedLegAmountsCents?.length??0)===0)return`GALE ${runtime.galeRuntime.currentStage} AGUARDANDO CONFIRMAÇÃO`;if(terminal&&['BET_EXECUTIONS','BET_ENTRIES','ALL_SOURCE_SIGNALS'].includes(terminal.strategySourceMode))return runtime.galeRuntime.currentStage===0?'BASE NA PRÓXIMA RODADA':`GALE ${runtime.galeRuntime.currentStage} NA PRÓXIMA RODADA`;if(runtime.galeRuntime.followUp&&runtime.galeRuntime.followUpBehavior==='REPEAT_UNTIL_LOSS')return`${runtime.galeRuntime.currentStage===0?'BASE':`GALE ${runtime.galeRuntime.currentStage}`} PÓS-WIN`;return`${runtime.galeRuntime.currentStage===0?'BASE':`GALE ${runtime.galeRuntime.currentStage}`} AGUARDANDO`;}
...
function roundAnalysisIndicator(runtime:TerminalRuntime|undefined){const role=runtime?.gameStrategyRuntime.lastAnnotationRole;if(role==='WIN'||role==='LOSS')return{symbol:'●',label:'Rodada analisada e adicionada à sequência',tone:'text-success'};if(role==='TRIGGER'||role==='RELEASE_TRIGGER'){if(runtime?.galeRuntime.active){const stage=runtime.galeRuntime.currentStage===0?'BASE':`GALE ${runtime.galeRuntime.currentStage}`;if(runtime.galeRuntime.awaitingDynamicFirstGale){const remaining=Math.max(0,(runtime.galeRuntime.triggerLossStreakTarget??1)-runtime.resultAnalyzerState.currentLossStreak);return{symbol:'⌛',label:`${stage} aguardando mais ${remaining} LOSS antes de ser armada`,tone:'text-warning'};}if((runtime.galeRuntime.preparedLegAmountsCents?.length??0)>0){if(runtime.screenControllerState.status==='ERROR'||runtime.screenControllerState.status==='INVALID')return{symbol:'!',label:`${stage} armada, mas o bot visual falhou e não confirmou o preenchimento`,tone:'text-danger'};return runtime.screenControllerState.paused?{symbol:'!',label:`${stage} armada no plano, mas o controle do mouse está desativado`,tone:'text-danger'}:{symbol:'➜',label:`${stage} armada: preenchimento solicitado para apostar na próxima rodada`,tone:'text-warning'};}}return{symbol:'→',label:'A próxima rodada será analisada, sem aposta armada',tone:'text-brand'};}if(role)return{symbol:'×',label:'Rodada não analisável ou ignorada',tone:'text-muted'};return null;}
