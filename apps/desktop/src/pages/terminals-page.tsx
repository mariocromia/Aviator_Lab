  ...
  return <div className="p-5">
    ...
    <div className="grid gap-3 xl:grid-cols-2">
      {terminals.map(terminal => {
        ...
        const isUpdating=updateState?.status==='BACKGROUND'||updateState?.status==='FOREGROUND';
        ...
        return <Card key={terminal.id} className="overflow-hidden">
          ...
          {updateState&&updateState.status!=='UPDATED'&&<div className={`flex items-center justify-between border-b px-4 py-2 text-[9px] font-bold uppercase tracking-wider ${updateState.status==='ERROR'?'border-danger/30 bg-danger/10 text-danger':'border-warning/25 bg-warning/10 text-warning'}`}><span>{updateState.status==='PENDING'?'Atualização pendente':updateState.status==='BACKGROUND'?'Atualizando em segundo plano':updateState.status==='FOREGROUND'?'Atualizando agora':`Falha na atualização: ${updateState.error??'erro desconhecido'}`}</span>{updateState.status!=='ERROR'&&<span className="font-mono">{updateState.progress}%</span>}</div>}
...
function TerminalModal({ open, terminal, onClose }: { open: boolean; terminal: Terminal | null; onClose(): void }) {
  ...
  return <Modal title={terminal ? `Configurar ${terminal.name}` : 'Novo Terminal'} open={open} onClose={onClose}>
    <form onSubmit={submit} className="grid grid-cols-2 gap-4 p-5">
      ...
      <Field label="Bolinhas exibidas no histórico"><select name="historyDisplayLimit" defaultValue={Math.min(terminal?.historyDisplayLimit??200,store.terminalHistoryDisplayMax)}>{historyDotOptions(store.terminalHistoryDisplayMax).map(value=><option key={value} value={value}>{value} bolinhas</option>)}</select><p className="mt-1 text-[9px] text-muted">Limite visual global: {store.terminalHistoryDisplayMax}.</p></Field>
      <Field label="Rodadas analisadas"><select name="analysisRoundLimit" defaultValue={terminal?.analysisRoundLimit??1000}>{[200,500,1000,2000,5000,10000].map(value=><option key={value} value={value}>{value.toLocaleString('pt-BR')} rodadas</option>)}</select><p className="mt-1 text-[9px] text-muted">Mais rodadas aumentam a profundidade e o tempo do recálculo.</p></Field>
...
function OperationCombinationsEditor({combinations,onChange,betStrategies,betPlans}:{combinations:TerminalOperationCombination[];onChange(value:TerminalOperationCombination[]):void;betStrategies:Array<{id:string;name:string}>;betPlans:Array<{id:string;name:string}>}){
  ...
  return <fieldset className="col-span-2 rounded-lg border border-line bg-canvas p-4"><legend className="px-1 text-[10px] font-bold uppercase tracking-wider text-muted">Combinações operacionais</legend>
    ...
    <div className="mt-3 space-y-3">{combinations.map((combination,index)=><div key={combination.id} className="rounded-md border border-line bg-panel p-3">
      ...
      <div className="mt-3 grid grid-cols-2 gap-3"><label><span className="label">Gatilho inicial</span><select className="mt-1.5 w-full" value={combination.triggerType??'BET_STRATEGY'} onChange={event=>{const triggerType=event.target.value as TerminalOperationCombination['triggerType'];update(index,{triggerType,behavior:triggerType==='SEQUENCE_AI'?'RUN_ONCE':combination.behavior,sequenceAiConfig:triggerType==='SEQUENCE_AI'?(combination.sequenceAiConfig??{minWindow:2,maxWindow:12,minOccurrences:20,minConfidence:60}):combination.sequenceAiConfig})}}><option value="PATTERN">Digitar sequência W/L</option><option value="BET_STRATEGY">Usar estratégia de entrada</option><option value="SEQUENCE_AI">Inteligência sequencial</option></select></label>{(combination.triggerType??'BET_STRATEGY')==='PATTERN'?<label><span className="label">Sequência para entrar</span><input className="mt-1.5 w-full font-mono uppercase" value={combination.pattern??''} onChange={event=>update(index,{pattern:normalizePattern(event.target.value)})} placeholder="Ex.: LLLWWL, LW ou W"/></label>:(combination.triggerType??'BET_STRATEGY')==='BET_STRATEGY'?<label><span className="label">Estratégia de entrada</span><select className="mt-1.5 w-full" value={combination.betStrategyId} onChange={event=>update(index,{betStrategyId:event.target.value})}>{betStrategies.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label>:<div className="flex items-end pb-2 text-[9px] leading-4 text-muted">A IA aprende com o histórico da fonte e prevê o próximo W antes de armar a BASE.</div>}</div>
      {combination.triggerType==='SEQUENCE_AI'&&<div className="mt-3 grid grid-cols-3 gap-3 rounded-md border border-brand/25 bg-brand/10 p-3"><AiNumber label="Janela mínima" value={combination.sequenceAiConfig?.minWindow??2} min={1} max={12} onChange={value=>update(index,{sequenceAiConfig:{...(combination.sequenceAiConfig??defaultSequenceAiConfig()),minWindow:value}})}/><AiNumber label="Janela máxima" value={combination.sequenceAiConfig?.maxWindow??12} min={2} max={12} onChange={value=>update(index,{sequenceAiConfig:{...(combination.sequenceAiConfig??defaultSequenceAiConfig()),maxWindow:value}})}/><AiNumber label="Ocorrências mínimas" value={combination.sequenceAiConfig?.minOccurrences??20} min={2} max={100000} onChange={value=>update(index,{sequenceAiConfig:{...(combination.sequenceAiConfig??defaultSequenceAiConfig()),minOccurrences:value}})}/><AiNumber label="Confiança mínima (%)" value={combination.sequenceAiConfig?.minConfidence??60} min={50} max={99.9} step={0.1} onChange={value=>update(index,{sequenceAiConfig:{...(combination.sequenceAiConfig??defaultSequenceAiConfig()),minConfidence:value}})}/><AiNumber label="Máx. L atuais" value={combination.sequenceAiConfig?.maxCurrentLossStreak??3} min={0} max={12} onChange={value=>update(index,{sequenceAiConfig:{...(combination.sequenceAiConfig??defaultSequenceAiConfig()),maxCurrentLossStreak:value}})}/><AiNumber label="Consenso mínimo (%)" value={combination.sequenceAiConfig?.minContextAgreement??60} min={0} max={100} step={1} onChange={value=>update(index,{sequenceAiConfig:{...(combination.sequenceAiConfig??defaultSequenceAiConfig()),minContextAgreement:value}})}/><AiNumber label="Risco máx. ciclo (0 = auto)" value={combination.sequenceAiConfig?.maxFullCycleLossRisk??0} min={0} max={100} step={0.1} onChange={value=>update(index,{sequenceAiConfig:{...(combination.sequenceAiConfig??defaultSequenceAiConfig()),maxFullCycleLossRisk:value}})}/><div className="col-span-2 text-[9px] leading-4 text-blue-100">Proteção recomendada: máximo 3 L atuais, consenso mínimo de 60% e risco do ciclo em modo automático. Para BASE + G1 + G2 + G3, o limite automático é 5%.</div></div>}
      <div className="mt-3 grid grid-cols-2 gap-3"><label><span className="label">Reentrada após LOSS/Gale</span><select className="mt-1.5 w-full" value={combination.lossReentryType??'BET_STRATEGY'} onChange={event=>update(index,{lossReentryType:event.target.value as TerminalOperationCombination['lossReentryType']})}><option value="PATTERN">Esperar sequência W/L</option><option value="BET_STRATEGY">Usar estratégia de entrada</option><option value="IMMEDIATE">Entrar imediatamente</option></select></label>{(combination.lossReentryType??'BET_STRATEGY')==='PATTERN'?<label><span className="label">Sequência para reentrar</span><input className="mt-1.5 w-full font-mono uppercase" value={combination.lossReentryPattern??''} onChange={event=>update(index,{lossReentryPattern:normalizePattern(event.target.value)})} placeholder="Ex.: W, LW ou LLW"/></label>:(combination.lossReentryType??'BET_STRATEGY')==='BET_STRATEGY'?<label><span className="label">Estratégia após LOSS</span><select className="mt-1.5 w-full" value={combination.lossReentryBetStrategyId??combination.betStrategyId} onChange={event=>update(index,{lossReentryBetStrategyId:event.target.value})}>{betStrategies.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label>:<div className="flex items-end pb-2 text-[9px] text-muted">O próximo estágio será confirmado no sinal seguinte.</div>}</div>
...
function defaultSequenceAiConfig(){return{minWindow:2,maxWindow:12,minOccurrences:20,minConfidence:60,maxCurrentLossStreak:3,minContextAgreement:60,maxFullCycleLossRisk:0};}
