...
export const createTerminalInputSchema = z.object({
  ...
  strategySourceMode:z.enum(['GAME_SIGNALS','ALL_SOURCE_SIGNALS','BET_EXECUTIONS','BET_ENTRIES']).default('GAME_SIGNALS'),
  ...
  postWinSkipSignals:z.number().int().min(0).max(10_000).default(0),
...
const triggerGroupsSchema=z.array(z.array(multiplierConditionSchema).min(1)).min(1).max(100);
export const gameStrategyConfigSchema = z.object({trigger:z.union([z.array(multiplierConditionSchema).min(1),triggerGroupsSchema]),win:z.array(multiplierConditionSchema).min(1),loss:z.array(multiplierConditionSchema).min(1),afterLoss:z.array(multiplierConditionSchema),release:z.array(multiplierConditionSchema).min(1),releaseConsecutiveCount:z.number().int().min(1).max(100).optional()});
