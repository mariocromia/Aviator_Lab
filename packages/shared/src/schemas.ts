...
const sequenceAiConfigSchema=z.object({minWindow:z.number().int().min(1).max(12).default(2),maxWindow:z.number().int().min(2).max(12).default(12),minOccurrences:z.number().int().min(2).max(100_000).default(20),minConfidence:z.number().min(50).max(99.9).default(60),maxCurrentLossStreak:z.number().int().min(0).max(12).default(3),minContextAgreement:z.number().min(0).max(100).default(60),maxFullCycleLossRisk:z.number().min(0).max(100).default(0)}).refine(value=>value.minWindow<=value.maxWindow,{message:'A janela mínima deve ser menor ou igual à janela máxima.'});
...
export const createTerminalInputSchema = z.object({
  ...
  analysisRoundLimit: z.number().int().min(200).max(10_000).default(1_000),
