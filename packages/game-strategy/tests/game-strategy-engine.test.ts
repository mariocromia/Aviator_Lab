...
describe('GameStrategyEngine', () => {
  ...
  it('releases only after the configured number of consecutive matching rounds', () => {
    const engine = new GameStrategyEngine();
    const sequenceConfig: GameStrategyConfig = { ...config, release: [{ operator: 'LT', value: 2 }], releaseConsecutiveCount: 3 };
    let runtime = initialRuntime();
    const roles: string[] = [];
    const progress: number[] = [];

    for (const multiplier of [1.72, 1.25, 1.8, 2.1, 1.7, 1.6, 1.5, 2.4]) {
      const result = engine.process({ terminalId: crypto.randomUUID(), strategyId: crypto.randomUUID(), config: sequenceConfig, runtime, round: makeRound(multiplier) });
      runtime = result.runtime;
      roles.push(result.annotation.role);
      progress.push(runtime.releaseProgress);
    }

    expect(roles).toEqual(['TRIGGER', 'LOSS', 'IGNORED', 'IGNORED', 'IGNORED', 'IGNORED', 'RELEASE_TRIGGER', 'WIN']);
    expect(progress).toEqual([0, 0, 1, 0, 1, 2, 0, 0]);
  });
