...
export class GameStrategyEngine {
  process(input: GameStrategyProcessInput): GameStrategyProcessResult {
    ...
    const runtime = { ...input.runtime, processedRounds: input.runtime.processedRounds + 1, lastMultiplier: round.multiplier, releaseProgress: input.runtime.releaseProgress ?? 0 };
    ...
    if (stateBefore === 'SEARCH_TRIGGER') {
      ...
      const releaseRequired = Math.max(1, Math.trunc(config.releaseConsecutiveCount ?? 1));
      runtime.releaseProgress = matches(round.multiplier, config.release) ? runtime.releaseProgress + 1 : 0;
      if (runtime.releaseProgress >= releaseRequired) {
